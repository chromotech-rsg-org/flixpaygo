...
import { getUsers, setUsers, getProfiles } from '@/lib/storage';
import { User, UserRole } from '@/lib/types';
import { motion } from 'framer-motion';
import { Plus, Edit, Trash2, Save, X, Users as UsersIcon, Key } from 'lucide-react';
import { toast } from 'sonner';

export default function UsersPage() {
  const [users, setUsersState] = useState(getUsers().filter(u => u.role === 'superadmin'));
  const [editing, setEditing] = useState<User | null>(null);
  const [changingPw, setChangingPw] = useState<{ userId: string; newPw: string } | null>(null);
  const profiles = getProfiles('superadmin');
  ...
  const handleSave = () => {
    ...
    if (idx >= 0) allUsers[idx] = editing; else allUsers.push(editing);
    setUsers(allUsers);
    setUsersState(allUsers.filter(u => u.role === 'superadmin'));
    setEditing(null);
    toast.success('Usuário salvo!');
  };

  const handleDelete = (id: string) => {
    if (id === '1') { toast.error('Não é possível excluir o admin principal'); return; }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black uppercase tracking-tight">Usuários SuperAdmin</h1>
          <p className="text-muted-foreground text-sm">{users.length} usuários</p>
        </div>
        <button onClick={() => setEditing({ id: crypto.randomUUID(), role: 'superadmin', email: '', password: '', name: '' })}
