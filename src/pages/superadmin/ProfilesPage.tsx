...
const SUPERADMIN_PAGES = [
  'Dashboard', 'Tenants', 'Propostas', 'Assinaturas', 'Relatórios', 'Configurações', 'Planos Comerciais', 'Perfis', 'Usuários',
];
...
export default function ProfilesPage() {
  const [profiles, setProfilesState] = useState(getProfiles('superadmin'));
  const [editing, setEditing] = useState<Profile | null>(null);

  const handleSave = () => {
    if (!editing || !editing.name.trim()) { toast.error('Nome é obrigatório'); return; }
    saveProfile(editing);
    setProfilesState(getProfiles('superadmin'));
    setEditing(null);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Gerencie perfis e permissões do SuperAdmin</p>
        </div>
        <button onClick={() => setEditing({ id: crypto.randomUUID(), name: '', scope: 'superadmin', permissions: emptyPermissions(SUPERADMIN_PAGES) })}
        ...
        {profiles.map((profile, i) => (
          <motion.div key={profile.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            <p className="text-xs text-muted-foreground">
              {profile.permissions.filter(p => p.view).length} páginas com acesso
            </p>
          </motion.div>
        ))}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })}
                  className="w-full px-3 py-2.5 bg-secondary/50 border border-border rounded-lg text-sm focus:outline-none focus:border-primary" placeholder="Ex: Administrador, Operador..." />
              </div>
              <div>
                <label className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3 block">Permissões</label>
                        ...
                        <tr key={perm.page} className="border-b border-border/50 hover:bg-secondary/20">
                          ...
                          {(['view', 'create', 'edit', 'delete'] as const).map(f => (
                            <td key={f} className="p-2 text-center">
                              <input type="checkbox" checked={perm[f]} onChange={() => togglePerm(idx, f)}
